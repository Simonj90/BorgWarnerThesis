...
\end{split}
\end{equation}
where $ T_{w} $ is the track width (distance between the left and right wheels), $ a_{z} $ the lateral acceleration and $ CGH $ the height from the ground to the center of gravity. The torque that is an effect of the lateral acceleration in Equation \ref{eq:normal_with_lat_acc}, does not take into account that the center of gravity height will change during driving.
...
\begin{split}
	\%F_{zFront}\cdot \%F_{zLeft} = \%F_{zFrontLeft} \\
	F_{zR} = mg\cdot \frac{l_{r}}{l_{f}+l_{r}} + \frac{m\cdot a_{x}\cdot CGH}{l_{f}+l_{r}} \\
	F_{zR} = mg\cdot \frac{l_{r}}{l_{f}+l_{r}} + \frac{m\cdot a_{x}\cdot CGH}{l_{f}+l_{r}} \\	
\end{split}
\end{equation}
