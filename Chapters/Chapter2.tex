...
The bicycle model described earlier is simplified car model that only captures the main characteristics of car. The fact that the bicycle model only has one wheel per axis means that the forces on the two front wheels respectively the two back wheels will be the same. In reality, there are many factors that affect the two wheels different, e.g. during cornering, the weight will be transfered from the inner to the outer wheel. In the case when an FXD is used in a car, this will transfer torque from one shaft to the other. The effects of the FXD will be described further in a later part of the paper. A model that is slightly more complicated that tries to capture these lateral differences, is the two track model seen in \ref{two_track_model}. 
